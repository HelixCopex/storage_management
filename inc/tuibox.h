...
static int vec_reserve_(char **data, int *length, int *capacity, int memsz,
                        int n);
static int vec_reserve_po2_(char **data, int *length, int *capacity, int memsz,
                            int n);
static int vec_compact_(char **data, int *length, int *capacity, int memsz);
static int vec_insert_(char **data, int *length, int *capacity, int memsz,
                       int idx);
static void vec_splice_(char **data, int *length, int *capacity, int memsz,
                        int start, int count);
static void vec_swapsplice_(char **data, int *length, int *capacity, int memsz,
                            int start, int count);
static void vec_swap_(char **data, int *length, int *capacity, int memsz,
                      int idx1, int idx2);
...
#define CLICK_COMPARATOR(x, y, tmp)                                            \
  (u->click == tmp || (box_contains(x, y, tmp) && u->click == NULL))

#define HOVER_COMPARATOR(x, y, tmp) (box_contains(x, y, tmp))

#define LOOP_AND_EXECUTE(f, c)                                                 \
  do {                                                                         \
    vec_foreach(&(u->b), tmp, ind) {                                           \
      if (tmp->screen == u->screen && f != NULL &&                             \
 ...
 *   for mouse support.
 */
static void ui_new(int s, ui_t *u) {
  struct termios raw;
 ...
 *   strip this down.
 */
static int ui_add(int x, int y, int w, int h, int screen, char *watch,
                  char initial, func draw, func onclick, func onhover,
                  void *data1, void *data2, int canscroll, ui_t *u) {
 ...
 *   to the UI.
 */
static void ui_key(char *c, func f, ui_t *u) {
 ...
 *   screen.
 */
static void ui_draw_one(ui_box_t *tmp, int flush, ui_t *u) {
  char *buf, *tok;
  int n = -1;
  int row = 0;
  ...
  tok = strtok(buf, "\n");
  while (tok != NULL) {
    int y = tmp->y + row + (tmp->canscroll ? u->scroll : 0);

    if (tmp->x > 0 && tmp->x < u->ws.ws_col && y > 0 && y < u->ws.ws_row) {
      printf("\x1b[%d;%dH%s", y, tmp->x, tok);
    }
 ...
 *   opaque to the user.
 */
static void _ui_update(char *c, int n, ui_t *u) {
  ui_box_t *tmp;
  ui_evt_t *evt;
  int ind, x, y;
  char cpy[n], *tok;
...
static int ui_text(int x, int y, char *str, int screen, func click, func hover,
                   ui_t *u) {
